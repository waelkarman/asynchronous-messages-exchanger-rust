...
impl UdpServer {
    ...
    fn run(mode: Speed) -> (){
        ...
        let instance= Arc::new(UdpServer {
        ...
        loop{

            {
                ...
                while *generation == false {
                    generation = self.generation_condvar.wait(generation).unwrap();
                }
            }
            ...
            {    
                ...
                if let Some(gen_msg) = mq.pop_front() {
                    let (seq, msg_t, msg) = msg_pack::msg_unpack(String::from(gen_msg.clone()));
                    {
                        ...
                        let src = *self.src.lock().unwrap();
                        socket.send_to(gen_msg.as_bytes(), src).unwrap();
                    }
    ...
    fn message_handler_loop(&self) {
        loop{
            ...
            match out {
                Ok((size, src)) => {
                    {
                        let mut source = self.src.lock().unwrap() ;
                        *source = src;
                    }
                    {
                        let mut generation = self.generation.lock().unwrap();
                    ...
                    self.generation_condvar.notify_all();
                    ...
                    match msg_t {
                        MsgType::MSG => {
...

                            {
                                let socket = self.socket.lock().unwrap();
                                let src = *self.src.lock().unwrap();
                                socket.send_to(ack_msg.as_bytes(), src).unwrap();
                            }
                        }
                        MsgType::ACK =>{
